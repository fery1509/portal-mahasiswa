  ...
  return (
    <div className="h-screen flex overflow-hidden bg-white">
      ...
      <div className={`fixed inset-0 z-40 flex md:hidden ${sidebarOpen ? 'visible' : 'invisible'}`}>
        <div
          className={`fixed inset-0 bg-gradient-to-br from-gray-600 to-gray-800 ${
            sidebarOpen ? 'opacity-75' : 'opacity-0'
          } transition-opacity duration-300 ease-linear`}
          onClick={() => setSidebarOpen(false)}
        />
        <div
          className={`relative flex-1 flex flex-col max-w-xs w-full bg-kampus-primary text-white transform ${
            ...
            <div className="mt-6 bg-[#000e7c] rounded-xl shadow-lg p-6 text-white flex flex-col md:flex-row items-center justify-between">
              <div className="space-y-2">
                ...
                <p className="text-indigo-100 text-sm">
                  Berikut adalah beberapa tugas utama yang dapat Anda lakukan di sini:
                </p>
              </div>
            ...
            <div className="mt-6 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 max-w-7xl">
              {stats.map((stat, index) => (
                ...
                >
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold text-[#000e7c]">{stat.value}</div>
                    <div className="flex items-center mt-1">
                      ...
                      <span className="text-sm text-[#000e7c]/70 ml-2">{stat.description}</span>
                    </div>
                  </CardContent>
                </Card>
              ))}
